...
function inspectionEvent($scope, IHttp, $uibModalInstance, items, $filter, $timeout) {
    ...
    function init() {
        $scope.param = {};
        occurTime_ = new Date();
        IHttp.post('operation/searchCheckeventList', { docId: items.docId ,docType:3}).then(function(rs) {
            if (rs.data.resultCode !== '1') return;
            $scope.item = rs.data.resultList;
        });
        IHttp.post('basedata/searchAllCheckItem', { enable: 1,docType:3}).then(function(rs) {
            if (rs.data.resultCode !== '1') return;
    ...
    $scope.edit = function(row) {
        ...
        IHttp.post('operation/searchCheckeventItemList', { docId: items.docId, cheEventId: row.cheEventId ,docType:3}).then(function(rs) {
            if (rs.data.resultCode !== '1') return;
            $scope.list = rs.data.resultList;
        });
    }
    ...
    $scope.delete = function(id) {
        IHttp.post('operation/deleteCheckevent', { docId: items.docId, cheEventId: id,docType:3 }).then(function(rs) {
            if (rs.data.resultCode !== '1') return;
            init();
