    ...
    vm.time_watch = function(code, i) { // 处理麻醉事件 vm.event为html页面ng-init赋值的
        ...
        switch (code) {
            case 1:
                ...
                break;
            case 1001:
                updateEnterRoomTime(new Date(vm.startOper.anaesPacuRec.inTime).getTime(), $filter('filter')(vm.startOper.eventList, function(item) {
        ...
        anesRecordInter.verifyDrugOverTime(docId).then((rs) => {
            ...
            if (text) {
                ...
                $uibModal.open({
                    ...
                    backdrop: 'static',
                    size: 'ua',
                    resolve: { argu: { outTime: outTime ? outTime : '' } },
