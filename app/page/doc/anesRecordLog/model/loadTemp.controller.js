...
function loadTemp($rootScope, $scope, IHttp, $uibModalInstance, $timeout, items, toastr, auth, $filter) {
    ...
    function savePzx(list){
        ...
        $scope.szPzx = list;
        IHttp.post('operCtl/updMonitorConfig', angular.merge(list,{docType:1,regOptId:items.regOptId})).then(function(rs) {
            if (rs.data.resultCode !== '1') return;
        });
    }
    ...
    function saveJcx(list){
        ...
        $scope.szJcx = list;
        IHttp.post('basedata/saveAnaesMonitorConfig',angular.merge(list,{docType:1,regOptId:items.regOptId})).then(function(rs) {
            if (rs.data.resultCode !== '1') return;
        });
    }
