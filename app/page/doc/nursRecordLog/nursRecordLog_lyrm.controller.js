...
function NursRecordLogCtrl($rootScope, $scope, IHttp, select, $q, toastr, confirm, $window, $timeout, $uibModal, $filter, auth) {
    ...
    vm.optCareRecord={};
    vm.optCareRecord.optbodys=[0,0,0,0,0];
    vm.drainageTube2=[0,0,0,0,0,0,0,0];
    ...
    function initData(callback, type) {
        IHttp.post('document/searchOptCareRecordByRegOptId', { regOptId: regOptId, type: type })
            .then((rs) => {
                ...
                //if(!!vm.optCareRecord.optbodys && vm.optCareRecord.optbodys.length>0)
                //    vm.optbodys = vm.optCareRecord.optbodys.split(',');

                if(!!vm.optCareRecord.drainageTube2 && vm.optCareRecord.drainageTube2.length>0){
                    if(vm.optCareRecord.drainageTube2.indexOf(';')>-1){
                        vm.optCareRecord.drainageTube2Other = vm.optCareRecord.drainageTube2.split(';')[1];
                        vm.drainageTube2 = vm.optCareRecord.drainageTube2.split(';')[0].split(',');
                    }else{
                     vm.drainageTube2 = vm.optCareRecord.drainageTube2.split(',');    
                    }
    ...
    function submit(processState, type) {
        ...
        //vm.optCareRecord.optbodys = vm.optbodys;
        vm.optCareRecord.drainageTube2 = vm.drainageTube2.join();
        if(vm.optCareRecord.drainageTube2Other)
            vm.optCareRecord.drainageTube2=vm.optCareRecord.drainageTube2+";"+vm.optCareRecord.drainageTube2Other;
