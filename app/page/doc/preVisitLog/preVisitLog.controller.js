PreVisitLogCtrl.$inject = ['$rootScope', '$scope', 'IHttp', '$window', '$timeout', '$state', 'anesRecordServe', '$q', 'toastr', 'confirm', 'auth', '$filter', 'select'];

module.exports = PreVisitLogCtrl;

function PreVisitLogCtrl($rootScope, $scope, IHttp, $window, $timeout, $state, anesRecordServe, $q, toastr, confirm, auth, $filter, select) {
    let vm = this;
    ...
    //术中启动定时监测
    if (currRouteName == 'midVisitLog_syzxyy') {
    ...
    function initData() {
        IHttp.post('document/searchPreVisitByRegOptId', { 'regOptId': regOptId }).then(function(rs) {
            ...
            vm.preVisit.designedAnaesListTran = select.getDictObjList(vm.preVisit.designedAnaesList, vm.anaesMethodList)
    ...
    $timeout(function() {
        ...
        $scope.$watch('vm.preVisit.designedAnaesList', function(n, o) {
            vm.preVisit.designedAnaesListTran = select.getDictObjList(n, vm.anaesMethodList)
        }, true)
    }, 1000);
